...
from models import Extractor, MLP, CNNFeatureExtractor, MLPFeatureExtractor, LSTMFeatureExtractor, TfFeatureExtractor
import utils
import time
import os
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=64, help='batch size for training')  # 训练时的批量大小
    ...
    X_train = pd.read_csv('save/X_train_ESS.csv')
    y_train = pd.read_csv('save/y_train_ESS.csv')

    # 转换为PyTorch的张量
    X_train_tensor = torch.tensor(X_train.values, dtype=torch.float32).unsqueeze(dim=1).to(device)
    ...
    # clf = FastGradMLP(hidden_sizes=[980, 5]).to(device)

    # extr = MLPFeatureExtractor().to(device)
    # clf = FastGradMLP(hidden_sizes=[32, 5]).to(device)

    # extr = LSTMFeatureExtractor().to(device)
    # clf = FastGradMLP(hidden_sizes=[32, 5]).to(device)

    extr = TfFeatureExtractor().to(device)
    clf = FastGradMLP(hidden_sizes=[102, 5]).to(device)
    ...
    save_path = "save/result/ess_tf_DP_SGD.pth"
    ...
    if not os.path.exists(save_path):
        ...
        if args.test_mode == 'linear':
            ...
            torch.save({'X_train': X_train, 'y_train': y_train, 'X_test': X_test, 'y_test': y_test},
                       '%s/dp_delta_%.2e_std_%.2f_ESS_extracted.pth' % (
