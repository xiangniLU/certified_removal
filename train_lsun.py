...
from fast_grad_models import FastGradExtractor, FastGradMLP, MLPExtractor  # 导入自定义的快速梯度模型
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='Training an LSUN model')
    # parser.add_argument('--data-dir', type=str, required=True, help='directory for LSUN data')  # SVHN 数据集的路径
    parser.add_argument('--save-dir', type=str, default='save', help='directory for saving trained model')  # 保存训练模型的路径
    parser.add_argument('--batch-size', type=int, default=400, help='batch size for training')  # 训练时的批量大小
    parser.add_argument('--process-batch-size', type=int, default=400, help='batch size for processing')  # 处理时的批量大小
    parser.add_argument('--test-batch-size', type=int, default=1000, help='batch size for testing')  # 测试时的批量大小
    ...
    parser.add_argument('--num-filters', type=int, default=64, help='number of conv filters')  # 卷积层的滤波器数量
    ...
    # 设置 DataLoader 使用的参数
    kwargs = {'num_workers': 0, 'pin_memory': True}  # 设置 DataLoader 参数：工作进程数量和是否使用 pin 内存
    ...
    # 创建训练集和测试集的数据集对象
    train_dataset = CustomDataset(image_dir='save/lsun_/train', label_file='save/lsun_/train/labels.txt',
                                  transform=lsun_transform)
    test_dataset = CustomDataset(image_dir='save/lsun_/test', label_file='save/lsun_/test/labels.txt',
                                 transform=lsun_transform)
    # 数据加载器
    ...
    # CNN
    # extr = FastGradExtractor([3, args.num_filters, args.num_filters], 9, 1, 4, normalize=args.normalize).to(device)
    # # 创造两个卷积层，（[输入通道3，输出通道]，卷积核5*5，步数1，池化核2*2，特征不归一化）
    # clf = FastGradMLP([5 * 5 * args.num_filters, 2]).to(device)
    # 创造一个全连接层，5*5*输入通道，输出10
    ...
    class LSTMExtractor(nn.Module):
        def __init__(self, input_size=128*3, hidden_size=128, num_layers=2):
            super(LSTMExtractor, self).__init__()
            self.rnn = DPLSTM(input_size, hidden_size, num_layers, dropout=0.2, batch_first=True)

        def forward(self, x):
            batch_size = x.size(0)
            x = x.permute(0, 2, 3, 1)  # 将形状变为 (batch_size, 128, 128, 3)
            x = x.reshape(batch_size, 128, 128*3)  # 将形状变为 (batch_size, 128, 128*3)
    ...
    class TfExtractor(nn.Module):
        def __init__(self, input_dim=128*3, hidden_dim=64, num_layers=1):
        ...
        def forward(self, x):
            ...
            batch_size = x.size(0)
            x = x.permute(0, 2, 3, 1)  # 变为 (batch_size, 128, 128, 3)
            x = x.reshape(batch_size, 128, 128*3)  # 变为 (batch_size, 128, 3*128)
            x = self.encoder(x)  # 变为 (batch_size, 128, 3*128)
            x = torch.mean(x, dim=1)  # 平均池化 -> (batch_size, 3*128)
            x = self.relu(x)
            return x

    # transfromer
    extr = TfExtractor().to(device)
    clf = FastGradMLP(hidden_sizes=[3*128, 2]).to(device)
    ...
    save_path = "save/result/lsun_tf_DP_sgd.pth"
    # 如果模型文件不存在，则进行训练和保存
    ...
    # 如果模型文件存在，则加载模型并进行测试
        ...
        if args.test_mode == 'linear':
            ...
            torch.save({'X_train': X_train, 'y_train': y_train, 'X_test': X_test, 'y_test': y_test},
                       '%s/dp_delta_%.2e_std_%.2f_SVHN_extracted.pth' % ("save/result", args.delta, args.std))
        else:
            test(args, extr, clf, loss_fn, device, test_loader)
