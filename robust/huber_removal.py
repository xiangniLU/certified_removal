...
parser.add_argument('--delta', type=float, default=1.0, help='delta')
...
parser.add_argument('--verbose', action='store_true', default=True, help='verbosity in optimizer')
...
def lr_huber_grad(w, X, y, lam, delta=5.0, weight=None):
    z = batch_multiply(X, w).mul_(y)
    error = z-1
    # Huber 损失的梯度
    is_small_error = error.abs() <= delta
    grad_small = error * is_small_error
    grad_large = delta * torch.sign(error) * (~is_small_error)
    grad = grad_small + grad_large
    return X.t().mv(grad) + lam * X.size(0) * w


def lr_huber_batch_grad(w, X, y, lam, delta=5.0, weight=None):
    z = batch_multiply(X, w).mul(y)  # 计算 Xw 并按标签调整方向
    error = z - 1  # 计算误差
    # 判断哪些样本误差属于小误差范围
    is_small_error = error.abs() <= delta
    # 计算 Huber 梯度（两种情况）
    grad_small = error * is_small_error  # 小误差情况
    grad_large = delta * torch.sign(error) * (~is_small_error)  # 大误差情况
    grad = grad_small + grad_large  # 合并两部分梯度
    # 计算加权梯度
    if weight is not None:
        grad = grad * weight  # 按权重加权
    # 计算最终梯度 (batch-wise)
    batch_grad = X.t().mv(grad) / X.size(0)  # 归一化，使梯度与样本规模无关
    # 加上 L2 正则化项（不依赖 X.size(0)，防止梯度异常增大）
    return batch_grad + lam * w
...
def lr_huber_hessian_inv(w, X, y, lam, delta=1.0, batch_size=50000):
    z = batch_multiply(X, w).mul_(y)
    error = z-1  # 计算误差
    is_small_error = error.abs() <= delta  # 判断误差是否小于 delta
    #D = torch.where(is_small_error, torch.sigmoid(X.mv(w).mul_(y)) * (1 - torch.sigmoid(X.mv(w).mul_(y))),
                    # delta * torch.ones_like(error))  # 计算修正后的 D 矩阵
    D= torch.where(is_small_error, torch.ones_like(error), torch.zeros_like(error))
    H = None
    num_batch = int(math.ceil(X.size(0) / batch_size))
    for i in range(num_batch):
        lower = i * batch_size
...
def ovr_huber_loss(w, X, y, lam, delta=5.0, weight=None):
    # 计算 logits，并只保留 y=1 的类别
    z = batch_multiply(X, w).mul_(y)
    # 计算误差
    error = 1 - z
    # Huber 损失计算
    ...
    large_error_loss = delta * (error.abs() - 0.5 * delta)
    # 计算 Huber 损失
    huber_loss = torch.where(is_small_error, small_error_loss, large_error_loss)
    # 若提供 weight，对损失加权
    if weight is not None:
        loss = huber_loss.mul_(weight).sum()
    # 添加 L2 正则项
    return loss + (lam / 2) * w.pow(2).sum()
...
def ovr_h_lr_optimize(X, y, lam, weight=None, b=None, num_steps=100, tol=1e-10, verbose=False, delta=1.0):
    w = torch.autograd.Variable(torch.zeros(X.size(1), y.size(1)).float().to(device), requires_grad=True)

    def closure():
        if b is None:
            return ovr_huber_loss(w, X, y, lam, delta, weight)  # 使用 Hubér 损失
        else:
            return ovr_huber_loss(w, X, y, lam, delta, weight) + (b * w).sum() / X.size(0)

    optimizer = optim.LBFGS([w], tolerance_grad=tol, tolerance_change=1e-10)
    for i in range(num_steps):
        optimizer.zero_grad()
        loss = ovr_huber_loss(w, X, y, lam, delta, weight)  # 使用 Hubér 损失
        if b is not None:
            loss += (b * w).sum() / X.size(0)
        loss.backward()
        if verbose:
...
# def ovr_h_lr_optimize(X, y, lam, weight=None, b=None, num_steps=500, verbose=False, delta=5.0):
#     """
#     训练 One-vs-Rest (OVR) 逻辑回归模型，使用 Huber 损失，并引入扰动 b
#     """
#     # 初始化参数 w
#     w = torch.autograd.Variable(torch.zeros(X.size(1), y.size(1)).float().to(device), requires_grad=True)
#     # 选择优化器 (Adam 适合 Huber 损失)
#     optimizer = optim.Adam([w], lr=0.1)
#     for i in range(num_steps):
#         optimizer.zero_grad()
#         # 计算 Huber 损失
#         loss = ovr_huber_loss(w, X, y, lam, delta, weight)
#         # 如果提供了 b，则加入扰动项
#         if b is not None:
#             if weight is None:
#                 loss += (b * w).sum() / X.size(0)
...
#         optimizer.step()
#         if verbose:
#             print(f'Iteration {i + 1}: loss = {loss.item():.6f}, grad_norm = {w.grad.norm():.6f}')
#
#     return w.data  # 返回最终优化后的 w
...
if os.path.exists(save_path):  # 检查模型是否已经存在
    ...
    if args.train_mode == 'ovr':
        ...
        if args.train_sep:
            ...
            # # 一次性联合训练所有二分类模型
            w = ovr_h_lr_optimize(X_train, y_train_onehot, args.lam, weight, b=b, num_steps=args.num_steps, verbose=args.verbose, delta=args.delta)
...
# 加载之前保存的非典型样本和典型样本索引
non_typical_samples = torch.load('robust/non_typical_samples_sst.pth')
    ...
    if args.train_mode == 'ovr':
        ...
        for k in range(y_train_onehot.size(1)):
            if weight is None or weight[i, k] > 0:
                ...
                H_inv = lr_huber_hessian_inv(w_approx[:, k], X_rem, y_rem, args.lam, delta=args.delta)
                #grad_i = lr_grad(w_approx[:, k], X_train_reordered[i].unsqueeze(0), y_train_reordered[i, k].unsqueeze(0), args.lam)# 计算梯度
                grad_i = lr_huber_grad(w_approx[:, k],
                                       X_train_reordered[i].unsqueeze(0),
                                       y_train_reordered[i, k].unsqueeze(0),
                                       args.lam,
                                       delta=args.delta)
                total_grad = grad_i.norm()
                # print("Total gradient (L2 norm):", total_grad.item())
                # 更新K矩阵，减去样本对K的贡献
                if weight is None:
                ...
                Accum_True_RGN_sum = lr_huber_batch_grad(w_approx[:, k], X_rem, y_rem, args.lam, delta=args.delta) + b[:, k]
