...
def load_features(args):
    ckpt_file = '%s/%s_%s_extracted.pth' % ("save/result", args.extractor, args.dataset)
...
def per_example_gradient(extr, clf, x, y, loss_fn, include_linear=False):
    ...
    gradients = []
    # for name, param in extr.named_parameters():
    #     print(f"Parameter name: {name}")
    #     print(f"Parameter shape: {param.shape}")

    # for module in extr.modules():   #MLP
    #     # print("weight gradient:", module.weight.grad.shape)
    #     if hasattr(module, 'weight') and isinstance(module.weight, torch.nn.Parameter):
    #         grad = module.weight.grad * x.size(0)
    #         gradients.append(grad.unsqueeze(0))
    #         # print("weight gradient:", module.weight.grad.shape)
    #     if hasattr(module, 'bias') and isinstance(module.bias, torch.nn.Parameter):
    #         gradients.append(module.bias.grad.view(1, -1) * x.size(0))
    #         # print("bias gradient:", module.bias.grad.shape)


    for name, param in extr.named_parameters():
        if param.requires_grad:
            if param.grad is not None:
                grad = param.grad * x.size(0)
                gradients.append(grad.unsqueeze(0))
            #     print(f"{name} gradient shape: {grad.shape}")
            # else:
            #     print(f"Gradient for {name} is None")

    # for idx, (name, param) in enumerate(extr.named_parameters()):
    #     if idx < 2:
    #         continue  # 跳过前两个参数
    #     if param.requires_grad:
    #         grad = param.grad * x.size(0)
...
#             # print("1Expanded weight gradient:", module.expanded_weight.grad)
#             # print("1Expanded bias gradient:", module.expanded_bias.grad)
...
#                 # print(module.expanded_bias.shape)
...
# def per_example_gradient(extr, clf, input_ids, attention_mask, target, loss_fn, include_linear=False):
#     # 获取分类器的输出
#     logits, activations, linearCombs = clf(extr(input_ids=input_ids, attention_mask=attention_mask))
#     # 计算损失
...
#     # 提取梯度信息
#     gradients = []
#     for module in extr.modules():
#         grad = module.weight.grad * x.size(0)
#         gradients.append(grad.unsqueeze(0))
#         if module.bias is not None:
#             gradients.append(module.bias.grad.view(1, -1) * x.size(0))
#     # 如果需要，进行额外的操作，比如使用goodfellow_backprop
#     if include_linear:
#         linearGrads = torch.autograd.grad(loss, linearCombs)
#         linearGrads = goodfellow_backprop(activations, linearGrads)
#         gradients += list(linearGrads)
#
#     # 打印每个参数的梯度形状
...
def clip_and_sum_gradients(gradients, C):
    ...
    # print(f"grad_vec1 size: {grad_vec.size()}")
    ...
    # print(f"grad_vec size2 {grad_vec.size()}")
...
def batch_grads_to_vec(parameters):
    if not parameters:
        raise ValueError("The parameters list is empty.")
    N = parameters[0].shape[0]
    vec = []
    for param in parameters:
        # print("param:",param.size())
        vec.append(param.view(N, -1))
        # print("param.view(N, -1)",param.view(N, -1).size())
