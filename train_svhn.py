...
def  main():
    ...
    parser.add_argument('--process-batch-size', type=int, default=500, help='batch size for processing')  # 处理时的批量大小
    ...
    # 设置 DataLoader 使用的参数
    kwargs = {'num_workers': 2, 'pin_memory': True}  # 设置 DataLoader 参数：工作进程数量和是否使用 pin 内存

    svhn_transform = transforms.Compose([
        transforms.ToTensor(),  # 将数据转换为张量
        transforms.Normalize((0.5, 0.5, 0.5), (1.0, 1.0, 1.0)),  # 对数据进行归一化
    ])

    # 加载训练集、额外集和测试集
    trainset = torchvision.datasets.SVHN(root=args.data_dir, split='train', download=True, transform=svhn_transform)
    extraset = torchvision.datasets.SVHN(root=args.data_dir, split='extra', download=True, transform=svhn_transform)
    trainset = torch.utils.data.ConcatDataset([trainset, extraset])
    testset = torchvision.datasets.SVHN(root=args.data_dir, split='test', download=True, transform=svhn_transform)
    ...
    # clf = FastGradMLP(hidden_sizes=[32, 10]).to(device)

    # transfromer
    # extr = TfExtractor().to(device)
    # clf = FastGradMLP(hidden_sizes=[96, 10]).to(device)
