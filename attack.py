...
def get_cmd_arguments():
    ...
    parser.add_argument('--dataset', default='CIFAR10', type=str, choices=['CIFAR10', 'MNIST'],
                        help='Which dataset to use (CIFAR10 or MNIST)')
    ...
    parser.add_argument('--need_topk', action='store_true',
                        help='Flag to enable using Top 3 posteriors for attack data')
...
def create_attack(dataset, dataPath, modelPath, trainTargetModel, trainShadowModel, need_augm, need_topk, param_init,
                  verbose):
    dataset = dataset
    need_augm = need_augm
    verbose = verbose
