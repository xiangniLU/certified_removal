...
def ovr_huber_loss(w, X, y, lam, delta=1.0, weight=None):
    """
    计算一对其余 (One-vs-Rest) 模型的 Huber 损失函数，并加入 L2 正则化
    :param w: 权重矩阵，形状为 (特征数, 类别数)
    :param X: 输入特征矩阵，形状为 (样本数, 特征数)
    :param y: 真实标签，形状为 (样本数, 类别数)，one-hot 编码
    :param lam: L2 正则化项的系数
    :param delta: Huber 损失的阈值
    :param weight: 可选，样本的权重，用于加权计算损失
    :return: 总损失
    """
    y_pred = X.mm(w)  # 获取原始 logits
    y_pred_prob = torch.softmax(y_pred, dim=1)  # 计算 softmax 概率

    # 计算误差
    error = y - y_pred_prob  # 真实概率 - 预测概率
    ...
    loss = torch.where(is_small_error, small_error_loss, large_error_loss).sum(dim=1).mean()  # 计算每个样本的平均损失

    loss += lam * w.pow(2).sum() / 2  # 加上 L2 正则化项
...
if os.path.exists(save_path):  # 检查模型是否已经存在
    ...
    if args.train_mode == 'ovr':
        ...
        if args.train_sep:
            ...
            # # 一次性联合训练所有二分类模型
            w= ovr_h_lr_optimize(X_train, y_train_onehot, args.lam, weight, b=b, num_steps=args.num_steps,
                                verbose=args.verbose,delta=0.00000001)
    else:
...
# 加载之前保存的非典型样本和典型样本索引
non_typical_samples = torch.load('huber/non_typical_samples.pth')
