...
class FastGradConv2dFunction(torch.autograd.Function):
    ...
    def forward(ctx, weight_param, bias_param, batch_size=1):
        ...
        # print(
        #     f"FastGradConv1dFunction forward: weight shape {weight.shape}, bias shape {bias.shape if bias is not None else 'None'}")  # 添加调试信息
        return weight, bias
    ...
    def backward(ctx, weight_grad, bias_grad):
        ...
        # print(
        #     f"FastGradConv1dFunction backward: weight_grad shape {weight_grad.shape}, bias_grad shape {bias_grad.shape if bias_grad is not None else 'None'}")  # 添加调试信息
